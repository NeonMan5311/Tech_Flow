    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 px-4 py-10">
      <div className="w-full max-w-3xl rounded-3xl border border-slate-800 bg-slate-900 p-6">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-emerald-400">Create group</p>
            <h2 className="mt-2 text-2xl font-semibold text-white">Set up a new group</h2>
        ...
        <form onSubmit={submitGroup} className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-4">
            ...
            <div>
              <label className="text-xs text-slate-400">Search members</label>
              <div className="mt-1 flex gap-2">
                <input
                  value={query}
                  onChange={(event) => setQuery(event.target.value)}
                  className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-2 text-sm text-white outline-none focus:border-emerald-400"
                  placeholder="Search by name, username, or email"
                />
                <button
                  type="button"
                  onClick={searchUsers}
                  className="rounded-xl border border-slate-700 px-4 py-2 text-xs font-semibold text-slate-100 transition hover:border-slate-400"
                >
                ...
                {results.map((user) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs text-slate-400">{isSelected ? 'Added' : 'Add'}</span>
                    </button>
                  )
                })}
            ...
            <div>
              <p className="text-xs text-slate-400">Selected members</p>
              <div className="mt-2 flex flex-wrap gap-2">
                {selected.length ? (
                  selected.map((user) => (
                    <span key={user._id} className="rounded-full border border-emerald-500/40 bg-emerald-500/10 px-3 py-1 text-xs text-emerald-200">
                      {user.name}
                    </span>
                  ))
                ) : (
